...
public class DistributedCacheClient<K, V> implements CacheClient<K, V> {
    ...
    public Optional<CacheValue<V>> get(CacheKey<K> key) {
                ...
                if (!keys.isEmpty()) {
                    prefetchExecutor.submit(() -> {
                        ...
                        for (Map.Entry<CacheKey<K>, CacheValue<V>> bEntry : batch.entrySet()) {
                            ...
                            targetNode.put(new DefaultCacheEntry<>(bEntry.getKey(), bEntry.getValue(), Long.MAX_VALUE));
                        }
                    });
                }
            }
        ...
        if (value != null) {
            node.put(new DefaultCacheEntry<>(key, value, Long.MAX_VALUE));
        }
    ...
    public void put(CacheKey<K> key, CacheValue<V> value, int ttlSeconds) {
        ...
        CacheEntry<K, V> entry = new DefaultCacheEntry<>(key, value, expiresAtMs);
        node.put(entry);
    }
}
